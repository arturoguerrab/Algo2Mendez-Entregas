...
Cuando se invoca a la función leer_linea_ptr(), se genera un while para que se pueda ir leyendo carácter a carácter desde el stdin mediante la función getchar(). Esto se realiza hasta que encuentre el '\n' ingresado por el usuario o encuentre el EOF. Además, se establece como condición de salida el máximo lógico (ml) = -1, ya que este indicaría que hubo un error al reasignar memoria con el realloc.
